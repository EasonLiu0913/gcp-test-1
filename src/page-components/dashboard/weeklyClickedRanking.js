...
const WeeklyClickedRanking = ({ setTotalClick }) => {
  ...
  const getTrackingByClickedDaysAndTotal = async () => {
    setIsLoading(true)
    try {
      setData([])
      const params = {
      ...
      const result = await dashboardAPI.trackingByClickedDaysAndTotal(params)
      if (!result.success) return reportError({ errorNo: result.errorNo || 9999 })
      setTotalClick(result.data.totalClick)
      setData(result.data?.clickTrackings.slice(0, TRACKING_BY_CREATE_DAYS_PARAMS_DEFAULT.LimitTotalQty) ?? [])
    } catch (err) {
  ...
  useEffect(() => {
    getTrackingByClickedDaysAndTotal()
  }, [sortedIndex])
  ...
  return <section className={classReader('p-4 p-lg-5 rounded border', { DashboardStyle: 'weekly-clicked-highlight' })}>
    <div className={classReader('d-flex justify-content-between mb-4', { DashboardStyle: 'header-wrapper' })}>
      <div className={classReader('mr-5')}>
        ...
        <div className={classReader('text-secondary mt-1 mb-3')}>7 Days Clicked Ranking</div>
      </div>
      <div>
        <Button
          label="查更多"
    ...
    { data.length ? <>
      <div className={classReader('scrollbar-x relative-position pt-2')}>
        ...
        <div className={classReader('pb-3', { DashboardStyle: 'table' })}>
          <div className={classReader('text-secondary justify-content-center')}>最後點擊日</div>
          ...
          <div className={classReader('text-secondary justify-content-end')}>總點擊量</div>
          {
            data.map((item, index) => <Fragment key={index}>
              ...
              <div className={classReader('border-top d-block')}>
                <div className={classReader('d-flex align-items-center')}>
                  ...
                  <div className={classReader('overflow-hidden')}>
                    <div className={classReader('text-lg bold pt-0 text-break-all ellipsis')} name="title" title={item.title}>{item.title}</div>
                    <div className={classReader('pt-1 text-nowrap')} name="shortUrl" title={decodeURIComponent(item.shortUrl)}>{decodeURIComponent(item.shortUrl)}</div>
                  </div>
                </div>
              </div>
              ...
              <div
                className={classReader('border-top text-right justify-content-end')}>
                <Link className={classReader('primary')} href={`/analysis/effect/detail/${item.pairInfoId}`}>
...
export default WeeklyClickedRanking
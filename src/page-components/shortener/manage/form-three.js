  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (isEditMode) {
        ...
        // 因為量產短網址，實際建立與單一相同
        router.push(`/shortener/manage/edit/finish?url=${result.data}`)
      } else {
        const result = await shortenerAPI.create(data)
        if (!result.success) return reportError({ errorNo: result.errorNo || 9999 })
        router.push(`/shortener/manage/create/finish?url=${result.data}`)
      }
    } catch (err) {
      reportError(err)

...
const FormThree = ({
  ...
  const onSubmit = async () => {
    ...
    data.parameters = data.parameters.filter(item => item !== '')
    setIsSubmitting(true)
    dispatch({ type: 'loading/openLoading' })
    try {
      if (router.query.id) {
        let result = await shortenerIndividualAPI.editByParameter(data)
        if (!result.success) return reportError({ errorNo: result.errorNo || 9999 })
        router.push('/shortener/parameters/edit/finish')
      } else {
        let result = await shortenerIndividualAPI.createByParameter(data)
        if (!result.success) return reportError({ errorNo: result.errorNo || 9999 })
        router.push('/shortener/parameters/create/finish')
      } 
    } catch (err) {
      reportError(err)
  ...
  useEffect(() => {
    const destUrl = rules.required(getValues('destUrl'))
    ...
    if (!ogSiteName && !ogTitle && !ogDesc && !ogImage && destUrl) getShareData()
    else changeValue()
  }, [])
          ...
          <div className={classReader('col-12 col-sm-4')}>
            <div className={classReader({ ShortenerStyle: 'create-share-preview__img' }, 'bg-gray')}>
              <div 
                ...
                style={{ backgroundImage: `url(${shareImage === '-' ? '/imageNotFound.svg' : shareImage})` }}
              >

...
const FormThree = ({
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (router.query.id) {
        if (data?.oneOnOneFile) data.oneOnOneFile = data.oneOnOneFile[0]
        let result = await shortenerIndividualAPI.edit(data)
        if (!result.success) return reportError({ errorNo: result.errorNo || 9999 })
        router.push('/shortener/one-on-one/edit/finish')
      } else {
        data.oneOnOneFile = data.oneOnOneFile[0]
        let result = await shortenerIndividualAPI.create(data)
        if (!result.success) return reportError({ errorNo: result.errorNo || 9999 })
        router.push('/shortener/one-on-one/create/finish')
      } 
